...
class IbtracsReader:
    ...
    def _get_index_from_name(self, name, year):
        ...
        if name not in self.name_dict[year]:
            raise KeyError(f"Could not find {name} ({year}).")
        return self.name_dict[year][name]
...
or name and year pair (str and int, respectively). Got {tc_identifier!r}."
    ...
    def change_settings(self, interpolating=None, columns=None):
        """Preferable method of changing reader settings."""
        if interpolating is not None:
            self.interpolating = self._handle_interpolating_input(interpolating)
        if columns is not None:
            self.columns = self._handle_columns_input(columns)

    def lookup_date(self, time_in):
        """
        ...
        -------
        list
            A list with the IBTrACS indicies that have TCs at the input time

        Example Use
        -----------
        reader = IbtracsReader(ibtracs_fn)
        tc_idxs = reader.lookup_date("2005-08-28 18-00-00")
        tc = reader.read(tc_idxs[0])
        ...
        if not three_hourly:
            raise ValueError(f"Time must be three hourly, got {time_in!r}.")
        key = time_in[:-6]
