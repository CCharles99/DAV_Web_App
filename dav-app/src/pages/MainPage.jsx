...
function MainPage() {
  ...
  const isLayerVisibile = (layerID) => {
    return map.current.getLayoutProperty(layerID, 'visibility') === 'visible';
  }
  ...
  useEffect(() => {
    if (mapLoaded.current) {
      if (showDAV) { map.current.getSource('dav').updateImage({ url: BASE_URL_DAV + URL_PARAMS }) };
      if (showIR) { map.current.getSource('ir').updateImage({ url: BASE_URL_IR + URL_PARAMS }) };
    }
  }, [frame, view]);
